...
describe("P2P wire-protocol version gate", () => {
  ...
  // bump. Revert 39 → 38 and BOTH halves red: the v38 frame stops being
  // refused, and the v39 frame stops being admitted. The admitting half is
  // the reach-guard — without it "refuses v38" is also satisfied by a client
  // that refuses everything.
  it("refuses the previous wire protocol (v38) and admits its own (v39)", async () => {
    const refusing = makeGuest();
    await refusing.adapter.initialize();
    await refusing.conn.simulateData(setupFrameAt(38));
    ...
    await admitting.conn.simulateData(setupFrameAt(39));
